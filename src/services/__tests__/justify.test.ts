...
describe("Justify Service", () => {
  describe("justifyText", () => {
    it("Should justify lines to exactly 80 characters (except last line)", () => {
      const text: string =
        "This is an example of text that needs to be justified properly to ensure that each line reaches the specified maximum character limit.";
      const numberOfCharsMax: number = 80;
...
describe("splitTextIntoLines", () => {
  it("Should split text into multiple lines when exceeding character limit", () => {
    const text =
      "Hello world this is a test that needs to be split into multiple lines";
